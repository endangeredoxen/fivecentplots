#########################################################
# Figure layout defaults
#########################################################
fcp_params = {'ax_edge_color': '#aaaaaa',
              'ax_fill_color': '#eaeaea',
              'ax_size': [400, 400],  # [width, height]
              'box_divider_color': '#bbbbbb',
              'box_fill_color': '#ffffff',
              'box_group_title_edge_color': '#ffffff',
              'box_group_title_fill_color': '#ffffff',
              'box_group_title_font_size': 15,
              'box_group_title_font_color': '#666666',
              'box_group_title_font_style': 'normal',
              'box_group_title_font_weight': 'bold',
              'box_group_label_edge_color': '#aaaaaa',
              'box_group_label_font_size': 14,
              'box_group_label_font_color': '#666666',
              'box_group_label_font_style': 'normal',
              'box_group_label_font_weight': 'normal',
              'box_marker_size': 5,
              'cbar_size': 30,
              'dpi': 72,
              'engine': 'bokeh',
              'fig_edge_color': '#ffffff',
              'fig_fill_color': '#ffffff',
              'grid_major_color': '#ffffff',
              'grid_major_style': '-',
              'grid_major_width': 1.3,
              'grid_minor_color': '#ffffff',
              'grid_minor_style': '-',
              'grid_minor_width': 0.5,
              'label_color': 'k',
              'label_font_size': 18,
              'label_rc_edge_color': '#8c8c8c',
              'label_rc_fill_color': '#8c8c8c',
              'label_rc_font_size': 15,
              'label_rc_font_color': '#ffffff',
              'label_rc_font_style': 'normal',
              'label_rc_font_weight': 'bold',
              'label_rc_size': 30,
              'label_font_style': 'italic',
              'label_font_weight': 'bold',
              'legend_edge_color': '#ffffff',
              'legend_font_size': 14,
              'legend_points': 1,
              'legend_title': '',
              'lines_width': 1.2,
              'marker_size': 8,
              'marker_type': 'o',
              'tick_labels_major_font_size': 14,
              'tick_labels_major_z_font_size': 14,
              'tick_labels_color': '#000000',
              'ticks_major_color': '#ffffff',
              'ticks_minor_color': '#ffffff',
              'title_fill_color': '#ffffff',
              'title_edge_color': '#ffffff',
              'title_font_color': '#333333',
              'title_font_size': 24,
              'title_font_weight': 'bold',
              'title_wrap_edge_color': '#5f5f5f',
              'title_wrap_fill_color': '#5f5f5f',
              'title_wrap_font_size': 16,
              'title_wrap_font_color': '#ffffff',
              'title_wrap_font_style': 'normal',
              'title_wrap_font_weight': 'bold',
              'title_wrap_size': 30,
              'toolbar_location': 'below',
              'tools': 'pan,wheel_zoom,box_zoom,save,reset',
              'ws_label_tick': 15,
              }

#########################################################
# Color scheme
#########################################################

colors = []

####################################################
# Marker scheme
####################################################

markers = []
