...
class BaseLayout:
    def __init__(self, plot_func, **kwargs):
        ...
        for ax in self.ax + ['z']:
            ...
            if '2' in ax:
                ...
                axl = ''
            # if getattr(self, 'axes%s' % axl).scale.get(plot_num) in globals()['LOG%s' % ax[0].upper()] and \
    ...
    def init_white_space(self, **kwargs):
        ...
        self.ws_ax_leg = utl.kwget(kwargs, self.fcpp, 'ws_ax_leg', 20)
    ...
    def update_from_data(self, data):
        ...
        #     #         'sci_%s' % ax not in kwargs.keys():
        #     #     kwargs['sci_%s' % ax] = 'best'
        # utl.kwget(kwargs, self.fcpp, 'sci_%s' % ax, 'best'),
...
class LayoutMPL(BaseLayout):
    ...
    def get_element_sizes(self, data):
        ...
        #     if self.axes.scale.get(plot_num) in ['loglog', 'log']:
        #         ax.set_xscale('log')
        #         ax.set_yscale('log')
        #     elif self.axes.scale.get(plot_num) in LOGY:
        #         ax.set_yscale('log')
        #     elif self.axes.scale.get(plot_num) in LOGX:
        #         ax.set_xscale('log')
        #     elif self.axes.scale.get(plot_num) in ['symlog']:
        #         ax.set_xscale('symlog')
        #         ax.set_yscale('symlog')
        #     elif self.axes.scale.get(plot_num) in SYMLOGY:
        #         ax.set_yscale('symlog')
        #     elif self.axes.scale.get(plot_num) in SYMLOGX:
        #         ax.set_xscale('symlog')
        #     elif self.axes.scale.get(plot_num) in ['logit']:
        #         ax.set_xscale('logit')
        #         ax.set_yscale('logit')
        #     elif self.axes.scale.get(plot_num) in LOGITY:
        #         ax.set_yscale('logit')
        #     elif self.axes.scale.get(plot_num) in LOGITX:
        #         ax.set_xscale('logit')
        #     if self.axes.twin_x:
        #         if self.axes2.scale.get(plot_num) in LOGY:
        #             ax2.set_yscale('log')
        #         elif self.axes2.scale.get(plot_num) in SYMLOGY:
        #             ax2.set_yscale('symlog')
        #         elif self.axes2.scale.get(plot_num) in LOGITY:
        #             ax2.set_yscale('logit')
        #     if self.axes.twin_y:
        #         if self.axes2.scale.get(plot_num) in LOGX:
        #             ax3.set_xscale('log')
        #         elif self.axes2.scale.get(plot_num) in SYMLOGX:
        #             ax3.set_xscale('symlog')
        #         elif self.axes2.scale.get(plot_num) in LOGITX:
        ...
        #         axes[ia] = self.set_scientific(aa, ia, 0, 0)
        ...
        #                       ylim[1], y2inc))
            ...
            if self.tick_labels_minor_x.on:
                if self.ticks_minor_x.number is not None:
                    if self.axes.scale.get(plot_num) not in LOG_ALLX:
                        loc = None
            ...
            if self.tick_labels_minor_y.on:
                if self.ticks_minor_y.number is not None:
                    if self.axes.scale.get(plot_num) not in LOG_ALLY:
                        loc = None
            ...
            if self.tick_labels_minor_x2.on and ax3 is not None:
                if self.ticks_minor_x2.number is not None:
                    if self.axes2.scale.get(plot_num) not in LOG_ALLX:
                        loc = None
    ...
    def get_legend_position(self):
        ...
        self.legend.position[1] = self.axes.position[1] + \
            (offset_x + offset_box + self.legend.size[0] + self.right) / \
            self.fig.size[0]
    ...
    def set_axes_scale(self, ir, ic):
        ...
        for ax in axes:
            if ax.scale.get(plot_num) is None:
                continue
            else:
                if str(ax.scale.get(plot_num)) in LOGX:
                    ax.obj[ir, ic].set_xscale('log')
                elif str(ax.scale.get(plot_num)) in SYMLOGX:
                    ax.obj[ir, ic].set_xscale('symlog')
                elif str(ax.scale.get(plot_num)) in LOGITX:
                    ax.obj[ir, ic].set_xscale('logit')
                if str(ax.scale.get(plot_num)) in LOGY:
                    ax.obj[ir, ic].set_yscale('log')
                elif str(ax.scale.get(plot_num)) in SYMLOGY:
                    ax.obj[ir, ic].set_yscale('symlog')
                elif str(ax.scale.get(plot_num)) in LOGITY:
    ...
    def set_axes_ticks(self, ir, ic):
        ...
        def get_tick_position(ax, tp, xy, plot_num, loc='first', idx=0):
            """
            ...
            elif getattr(self, 'axes%s' % lab).scale.get(plot_num) in (LOGX if xy == 'x' else LOGY):
        ...
        for ia, aa in enumerate(axes):
            ...
            if ia == 0:
                if not skipx:
                    self.set_scientific(axes[ia], ir, ic)
            elif self.axes.twin_y or self.axes.twin_x:
                if not skipy:
                    self.set_scientific(axes[ia], ir, ic, 2)
            ...
            if self.tick_cleanup and tlmajx.on:
                ...
                xcx = get_tick_position(axes[ia], tp, 'x', plot_num, 'first', ia)
                xfx = get_tick_position(axes[ia], tp, 'x', plot_num, 'last', ia)
                ...
                if not skipx:
                    ...
                    if tp['x']['last'] != -999:
                        ...
                        if getattr(self, 'axes%s' % lab).scale.get(plot_num) not in LOG_ALLX:
            ...
            if self.tick_cleanup and tlmajy.on:
                ...
                ycy = get_tick_position(axes[ia], tp, 'y', plot_num, 'first', ia)
                yfy = get_tick_position(axes[ia], tp, 'y', plot_num, 'last', ia)
                ...
                if not skipy:
                    ...
                    if tp['y']['last'] != -999:
                        ...
                        if getattr(self, 'axes%s' % lab).scale.get(plot_num) not in LOG_ALLY:
            ...
            for axx in ax:
                ...
                if getattr(self, 'ticks_minor_%s' % axl).number is not None:
                    ...
                    if getattr(self, 'axes%s' % lab).scale.get(plot_num) not in (LOG_ALLX if axx == 'x' else LOG_ALLY):
                        loc = None
                ...
                if not self.separate_ticks and axl == 'x' and ir != self.nrow - 1 and self.nwrap == 0 or \
                    ...
                    if getattr(self, 'axes%s' % lab).scale.get(plot_num) in (LOGX if axx == 'x' else LOGY):
                    ...
                    if self.tick_cleanup and tlminon:
                        ...
                        if len(ticks) > 0:
                            ...
                            for i in range(0, number - 1):
                                if getattr(self, 'axes%s' % lab).scale.get(plot_num) in \
                        ...
                        if len(tp['x']['ticks']) > 0:
                            ...
                            xcxmin = get_tick_position(axes[ia], tp, axx, plot_num, first+m0, ia)
                        ...
                        if axx == 'x' and self.axes.scale.get(plot_num) in LOGX or \
                                axx == 'y' and self.axes.scale.get(plot_num) in LOGY:
    ...
    def set_scientific(self, ax, ir, ic, idx=0):
        """
        ...
        # Set labels
        logx = getattr(self, 'axes%s' % lab).scale.get(plot_num) in LOGX + SYMLOGX + LOGITX
        ...
        logy = getattr(self, 'axes%s' % lab).scale.get(plot_num) in LOGY + SYMLOGY + LOGITY
